...
CREATE FUNCTION [dbo].[kpi_no_briefings_part_part] 
(
	@sup_id int,
	@geo_id int,
	@sbu_id int,
...
BEGIN
	...
	if (( @geo_id is null OR @geo_id = 7 ) and @sbu_id is null and @pro_id is null)
		...
		select @result = count (distinct briefing.briefing_no + '##' + cast(sup_id as nvarchar))     
			from         
				briefing_supplier
				inner join briefing on briefing_supplier.brf_id = briefing.id
			where 
				brf_id in
					(select brf.id    
		...
		where 
			brf_id in
				(select brf.id 
				from briefing as brf INNER JOIN
					briefing_supplier brs ON brf.id = brs.brf_id
				where 
		...
		-- for kpi 2 / geo_id = 7 --> global / geo_id is not null
		if (@geo_id = 7)
			if (@sbu_id is null)
